...
chrome.alarms.onAlarm.addListener(() => { }); // Empty listener keeps SW active
...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  if (message.type === "form_submission" || message.type === "login_attempt") {
    ...
    // Send the data to the correct API
    let endpoint =
      message.type === "form_submission" ? "track-email" : "track-login";
    console.log("Sending to endpoint:", endpoint); // Debugging line
...
chrome.runtime.onMessage.addListener((message) => {
  if (message.type === "page_visit") {
    // Validate URL and timestamp
    if (!message.url || !message.timestamp) {
      console.error("Missing URL/timestamp in message:", message);
      return;
    }

    // 🔴 Add notification logic HERE (new code)
    if (message.isIncognito) {
      try {
        const urlHost = new URL(message.url).hostname; // Extract domain
    ...
    // Log the outgoing data for debugging
    const payload = {
      url: message.url,
      timestamp: message.timestamp,
      is_incognito: message.isIncognito || false, // Default to false if missing
    };
